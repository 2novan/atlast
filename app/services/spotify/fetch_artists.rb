module Spotify
  ...
  class FetchArtists
    ...
    # Spotify::FetchArtists.new(User.last).call
    def call
      # rails c -> reload!
      @spotify_user.top_artists(limit: 2, offset: 0, time_range: 'medium_term').each do |spotify_artist|
        artist = Artist.where({ name: spotify_artist.name, spotify_id: spotify_artist.id,
                                image_url: spotify_artist.images.last["url"] }).first_or_create
        sleep 0.05
        ...
        spotify_artist.albums.each do |release|
          ...
          # binding.pry
          release_date = if release.release_date =~ /\d{4}-\d{2}-\d{2}/
                           release.release_date
                         else
                           Date.new(
                             release.release_date.to_i, 1, 1
                           )
                         end
          album = Release.where({ name: release.name, album_type: release.album_type, artist_id: artist,
                                  image_url: release.images.last["url"], total_tracks: release.total_tracks, artist_id: artist, release_date: release_date, spotify_id: release.id }).first_or_create
          release.tracks.each do |track|
            sleep 0.05
            track = Track.where({ name: track.name, duration: track.duration_ms, track_number: track.track_number,
                                  spotify_id: track.id, release: album }).first_or_create!
